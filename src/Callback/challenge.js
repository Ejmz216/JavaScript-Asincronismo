...
function ObtenerDatos(urlApi, callback) {
    ...
    //El metodo .open realiza la petición de apertura de comunicación, el metodo puede ser 'GET' o 'POST', luego se envia la URL, si es asincrono (true o false), 
    //usuario y contraseña. En esta caso solo se utiliza el metodo, la url y async
...
//se invoca el metodo ObtenerDatos() pasandole como argumentos la varible API concatenada con la cadena 'products' para acceder a la URL de la API deseada, 
//y una función anónima que recibe 2 parámetros (un objeto de error y un arreglo que almacena todos los objetos traidos por la API).
ObtenerDatos(`${API}/products`, function (error1, data1) {
    ...
    //se invoca nuevamente la función ObtenerDatos con el fin de acceder a un objeto puntual del arreglo data1, se envia como parámetros la url de la API 
    //apuntando al atributo del primer objeto de arreglo data1 y nuevamente una función anónima.
    ObtenerDatos(`${API}/products/${data1[0].id}`, function (error2, data2) {
